...
/*
 * Wraps DDSPStaged() with a netf-only reactive trigger policy: only
 * re-pivots when IDA's error test fail counter (netf) has grown since the
 * last check.
 */
typedef struct
{
...
static int NetfSpecUpdate(DDStatePivot self,
                          sunrealtype t,
                          N_Vector Y,
                          void* user_data,
                          uint8_t* spec,
...
{
  NetfSpecContent* c = (NetfSpecContent*)self->content;
...
static void NetfSpecDestroy(DDStatePivot self)
{
  NetfSpecContent* c = (NetfSpecContent*)self->content;
...
static DDStatePivot NetfSpecCreate(DDStatePivot inner,
                                   void* ida_mem,
                                   FILE* stats_file)
{
  NetfSpecContent* content = malloc(sizeof(*content));
  ...
  sp->content      = content;
  sp->ops->update  = NetfSpecUpdate;
  sp->ops->destroy = NetfSpecDestroy;
...
int main(int argc, char* argv[])
{
  ...
  DDStatePivot sp = NetfSpecCreate(inner, ida_mem, stats_file);
  TEST_ASSERT(sp != NULL);
