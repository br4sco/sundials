 ...
 * pivot (a PIVPivot call where spec_changed == SUNTRUE) as the time between
 * pivot checks (dt_check) is varied. Solution output still happens every
 * tstep, but PIVPivot is only invoked once t has reached the next scheduled
    ...
    if (t >= t_next_check)
    {
      ...
      if (spec_changed)
      {
        ...
        DD_LOG(stats_file,
               ...
               (double)t,
               (double)dt_check);
        TEST_ASSERT(DD_PRINT_ALL_STATS(DDGetIDAMem(dd_mem),
                                       stats_file,
